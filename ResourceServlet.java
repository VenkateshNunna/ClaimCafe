...
import example.dataobject.Attachment;
import example.dataobject.Comment;
import example.dataobject.Post;
...
//			return Response.status(500).entity(output).build();
		...
		for(HashMap post : allPosts){
			...
			if(postId.startsWith(id)){
				...
				postObject.setId(postId);
				postObject.setMessage((String)post.get("Message"));
				postObject.setOwner((String)post.get("Owner"));
				...
				if(attachment!=null){
					for (Object key : attachment.keySet()) {
						...
						Attachment attachedObject = new Attachment();
						// set the content type of the attachment
						attachedObject.setContenttype(attach.get("content_type").toString());
						// append the document id and attachment key to the URL
						attachedObject.setUrl(URLTemplate_post + postId + "/" + key);
						// set the key of the attachment
						attachedObject.setKey(key + "");
	
						// add the attachment object to the array
						postObject.setAttachment( attachedObject);
					}
				}
				if(allComments != null && allComments.size() > 0){
					for(HashMap comment : allComments)
					{
						...
						if(commentId.startsWith(postId)){
							...
							commentObject.setId((String)commentId);
							commentObject.setMessage((String)comment.get("message"));
							commentObject.setOwner("Venkatesh");
							...
							if(attachment!=null){
								for (Object key : attachment.keySet()) {
									...
									attachedObject.setContenttype( attach.get("content_type").toString());
									// append the document id and attachment key to the URL
									attachedObject.setUrl( URLTemplate_post + commentId + "/" + key);
									// set the key of the attachment
									attachedObject.setKey( key + "");
				
									// add the attachment object to the array
									commentObject.setAttachment(attachedObject);
								}
							}
					...
					postObject.setComments(commentsList);
				}
...
// 		JsonObject jsonObject = new JsonObject();
//		jsonObject.addProperty("id", commentRec.get("_id") + "");
//		return Response.ok().entity(new GenericEntity<List<Post>>(posts) {}).build();
